...
- [x] P12 integration tests (11/11 vs real Postgres; FOUND+FIXED prod bug: drizzle sql-template array params broke email/calendar relink) · [x] P13 review complete (1 HIGH: calendar backfill stall — fixed; 1 MED: async shell blocking loading states — fixed; 2 LOW — fixed). PR open; post-deploy checklist pending Robb's merge.
